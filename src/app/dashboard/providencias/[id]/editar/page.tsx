...
import type { Cidadao, Categoria, Orgao, Providencia } from '@/types/database'
...
export default function EditarProvidenciaPage() {
  ...
  useEffect(() => {
    const loadProvidencia = async () => {
      ...
      try {
        ...
        // Super admin pode acessar qualquer providência
        if (!isSuperAdmin(user)) {
          query = query.eq('gabinete_id', tenant.id)
        }

        const { data: provData, error: provError } = await query.single()

        if (provError) throw provError

        if (provData) {
          setProvidencia(provData as Providencia)
          setFormData({
            titulo: provData.titulo || '',
            descricao: provData.descricao || '',
            categoria_id: provData.categoria_id || '',
            orgao_destino_id: provData.orgao_destino_id || '',
            prioridade: provData.prioridade || 'media',
            localizacao_tipo: provData.localizacao_tipo || '',
            localizacao_descricao: provData.localizacao_descricao || '',
            prazo_estimado: provData.prazo_estimado || '',
            observacoes_internas: provData.observacoes_internas || '',
            endereco: provData.endereco || '',
            bairro: provData.bairro || '',
            latitude: provData.latitude?.toString() || '',
            longitude: provData.longitude?.toString() || '',
          })
          if (provData.cidadao) {
            setSelectedCidadao(provData.cidadao as Cidadao)
          }
        }
    ...
    loadProvidencia()
  }, [tenant, id, supabase, user, router])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Super admin pode atualizar qualquer providência
      if (!isSuperAdmin(user)) {
        query = query.eq('gabinete_id', tenant.id)
      }
      ...
      router.push(`/dashboard/providencias/${id}`)
    } catch (error) {
      console.error('Error updating providencia:', error)
      toast.error('Erro ao atualizar providência')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '36px' }}>
        <Link href={`/dashboard/providencias/${id}`} style={{ textDecoration: 'none' }}>
          <button style={{
            width: '48px',
            height: '48px',
            borderRadius: '14px',
          ...
          <div style={cardContentStyle}>
            ...
            <div style={fieldGroupStyle}>
              ...
              <textarea
                ...
                placeholder="Ex: Próximo ao número 123, em frente à escola municipal..."
